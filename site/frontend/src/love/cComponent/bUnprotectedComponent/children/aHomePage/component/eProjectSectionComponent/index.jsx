        ...
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          ...
          <div className="flex mx-auto mb-10 justify-center">
            <ScrollArea className="w-80 lg:w-2/3 whitespace-nowrap rounded-md">
              {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
                <a 
                  ...
                  ${activeProjectGroup === each.title ? 
                    "bg-[#4831D4] dark:bg-[#CCF381] text-[#CCF381] dark:text-[#4831D4] border-[#CCF381] dark:border-[#4831D4] rounded-t" : 
              ...
              <ScrollBar orientation="horizontal" className="bg-[#4831D4] dark:bg-[#CCF381]" />
            </ScrollArea>
          </div>
          
          {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
            <React.Fragment key={index}>
              <div className={`flex flex-wrap text-center bg-[#4831D4] dark:bg-[#CCF381] text-[#CCF381] dark:text-[#4831D4] bg-opacity-60 px-4 py-6 rounded-lg ${activeProjectGroup === each.title ? "" : "hidden"}`}>
                ...
                {each?.projects?.map((each1, index1) => (
                  <React.Fragment key={index1}>
                    <div className="p-4 w-full md:w-1/3 text-left">
                      <div className="h-full bg-[#CCF381] dark:bg-[#4831D4] text-[#4831D4] dark:text-[#CCF381] rounded-lg overflow-hidden">
                          ...
                          <div className="flex items-center flex-wrap ">
                            ...
                            {each1?.links?.map((each2, index2) => (
                              each2?.title === "Visit Application" ? (
                                <span key={index2} className="mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm py-1 border-r-2 border-[#4831D4] dark:border-[#CCF381]">
                              ...
                              each2?.title === "Visit Admin" ? (
                                <span key={index2} className="mr-3 inline-flex items-center leading-none text-sm py-1 border-r-2 border-[#4831D4] dark:border-[#CCF381]">
                                  <Link to={each2?.url} target="_blank" rel="noreferrer" >
